...
int main(void) {
    ...
    error = cudaMemcpy(device_array_a,
                    host_array_a,
                    BUF_32M * sizeof(*device_array_a), 
                    cudaMemcpyHostToDevice);
    if (error != cudaSuccess)
        exit(EXIT_FAILURE);

    error = cudaMemcpy(device_array_b, 
                    host_array_b,
                    BUF_32M * sizeof(*device_array_b), 
                    cudaMemcpyHostToDevice);
    if (error != cudaSuccess)
        exit(EXIT_FAILURE);
	...
	{
        error = cudaMemcpy(device_array_c, 
                        device_array_b + i,
        ...
        error = cudaMemcpy(device_array_b + i, 
                            device_array_a + i,
        ...
        error = cudaMemcpy(device_array_a + i, 
                            device_array_c,
    ...
    error = cudaMemcpy(host_array_a, 
                        device_array_a,
                        BUF_32M * sizeof(*host_array_a), 
                        cudaMemcpyDeviceToHost);
    if (error != cudaSuccess)
        exit(EXIT_FAILURE);
    
    error = cudaMemcpy(host_array_b, 
                        device_array_b,
                        BUF_32M * sizeof(*host_array_b), 
                        cudaMemcpyDeviceToHost);
    if (error != cudaSuccess)
        exit(EXIT_FAILURE);
